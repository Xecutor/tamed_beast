...
template<
    ...
    class Send>
void handle_request(boost::beast::string_view doc_root, request_resolver& req_resolver, http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send)
{
    ...
    if(hnd) {
        ...
        struct http_responder_impl: public http_responder{
            ...
            std::string& contentType;
            bool& notFound;
            http_responder_impl(std::string& argPath, std::string& argData, std::string& argError, std::string& argContentType, bool& argNotFound):
...
public:
    ...
    plain_websocket_session(tcp::socket socket, web_server::ws_handler req_handler)
        : websocket_session<plain_websocket_session>(
            socket.get_executor().context(), std::move(req_handler))
...
public:
    ...
    ssl_websocket_session(ssl_stream<tcp::socket> stream, web_server::ws_handler req_handler)
        : websocket_session<ssl_websocket_session>(
            stream.get_executor().context(), std::move(req_handler))
